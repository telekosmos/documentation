 ...
 * @aparam {Boolean} outputCode true to, in fact, infer the source code; false not to do
...
function inferSourceCode(source/*: ?string*/, outputCode/*: Boolean*/) {
  ...
  return function inferCode(comment/*: Comment */) {
    ...
    comment.context['blockSource'] = source.split('\n').slice(ini-1, end).join('\n');
    comment.context.blockSource = comment.context.blockSource.replace(/\t/g, '    '); // .replace(/    /g, '  ');
